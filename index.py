...
def addProyectos():
    if request.method == 'POST':
        ...
        cur = conexion.cursor()
        cur.execute(
            'INSERT INTO proyectos (idp, nombrep) VALUES (%s, %s)', (codigoP, nombreP))
        conexion.commit()
...
def addItems():
    if request.method == 'POST':
        ...
        valoru = request.form['valoru']
        cur = conexion.cursor()
        cur.execute('INSERT INTO items (idi, nombrei, titulo, unidad, valoru) VALUES (%s, %s, %s, %s, %s)',
                    (codigoI, nombreI, titulo, unidad, valoru))
        conexion.commit()
...
def addUsuarios():
    if request.method == 'POST':
        ...
        nombrep = request.form['nombrep']
        cur = conexion.cursor()
        cur.execute('INSERT INTO usuarios (idu, nombreu, emailu, idp, nombrep) VALUES (%s, %s, %s, %s, %s)',
                    (idu, nombreu, emailu, idp, nombrep))
        conexion.commit()
